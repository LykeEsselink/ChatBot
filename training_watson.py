...
# Sends natural language query to the watson discovery service with the discovery API
...
# Can be used to rate watson's responses to train the model
...
# Open the training questions and print them out
...
# Creates a list of responses for the training queries
query_responses = [None] * 52
for i, sents in enumerate(wq):
    query_responses[i] = send_query(sents)

# Print all the responses so they can be rated manually
